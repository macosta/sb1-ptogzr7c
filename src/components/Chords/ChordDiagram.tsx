...
const getChordPositions = (rootNote: string, chordType: 'Major' | 'Minor'): ChordPosition[] => {
  ...
  // For demo purposes, we'll just show positions for the selected chord type
  ...
  // Default chord position - just a placeholder
  return [
    {
      fretStart: 0,
...
const ChordDiagram: React.FC<ChordDiagramProps> = ({ className, rootNote = 'C', chordType = 'Major' }) => {
  // Get chord positions based on selected root note and chord type
    ...
    <div className={cn("bg-white dark:bg-metal-darker rounded-lg shadow-md p-6", className)}>
      ...
      <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-4">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-2">
          {chordType === 'Major' ? (
            <>The {rootNote} Major chord consists of the notes {rootNote} (root), {/* 3rd */}, and {/* 5th */} (fifth). These notes form the {rootNote} major triad.</>
          ) : (
            <>The {rootNote} Minor chord consists of the notes {rootNote} (root), {/* minor 3rd */}, and {/* 5th */} (fifth). These notes form the {rootNote} minor triad.</>
          )}
        </p>
