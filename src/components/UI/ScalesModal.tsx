...
// Function to determine scale type from name
...
// Get description based on scale type
const getScaleDescription = (scaleType: string): string => {
  switch (scaleType) {
    case 'Major (Ionian Mode)':
      return "The major scale (Ionian mode) is the foundation of Western music theory. It has a bright, happy sound.";
    case 'Minor (Aeolian Mode)':
      return "The natural minor scale (Aeolian mode) has a darker, more melancholic sound compared to the major scale.";
    case 'Harmonic Minor':
      return "The harmonic minor scale raises the 7th degree of the natural minor scale, creating a stronger pull to the tonic.";
    case 'Melodic Minor':
      return "The melodic minor scale raises both the 6th and 7th degrees when ascending, but uses the natural minor when descending.";
    case 'Major Pentatonic':
      return "The major pentatonic scale is a five-note scale derived from the major scale. It has a bright, simple sound that works well in many styles.";
    case 'Minor Pentatonic':
      return "The minor pentatonic scale is a five-note scale derived from the minor scale. It's widely used in rock, blues, and many other genres.";
    case 'Blues':
      return "The blues scale adds a flat 5th (blue note) to the minor pentatonic scale, giving it that characteristic blues sound.";
    case 'Dorian Mode':
      return "The Dorian mode is the second mode of the major scale. It's a minor scale with a raised 6th, giving it a slightly brighter sound.";
    case 'Phrygian Mode':
      return "The Phrygian mode is the third mode of the major scale. It's a minor scale with a lowered 2nd, giving it a Spanish or Middle Eastern flavor.";
    case 'Lydian Mode':
      return "The Lydian mode is the fourth mode of the major scale. It's a major scale with a raised 4th, giving it a dreamy, floating quality.";
    case 'Mixolydian Mode':
      return "The Mixolydian mode is the fifth mode of the major scale. It's a major scale with a lowered 7th, common in rock, jazz, and folk music.";
    case 'Locrian Mode':
      return "The Locrian mode is the seventh mode of the major scale. It's a diminished scale rarely used as a tonal center in Western music.";
    default:
      return "A pattern of whole and half steps that forms the foundation of melodies and harmonies.";
  }
};

// Function to get common chords for a scale
...
const ScalesModal: React.FC<ScalesModalProps> = ({ isOpen, onClose }) => {
  ...
  // Get filtered scales based on selected root note
  const getFilteredScales = (categoryIndex: number) => {
    const category = scaleCategories[categoryIndex];
    return selectedNote 
      ? filterScalesByRoot(category.scales, selectedNote)
      : category.scales;
  };
  
  // Get the count for a category based on filtered scales
  const getCategoryCount = (categoryIndex: number) => {
    const filteredScales = getFilteredScales(categoryIndex);
    return filteredScales.length;
  };
